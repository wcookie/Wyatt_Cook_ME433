...
void __ISR(_I2C_2_VECTOR, IPL1SOFT) I2C2SlaveInterrupt(void) {
  ...
  if(I2C2STATbits.D_A) {       // received data/ACK, so Case (3) or (4)
    if(I2C2STATbits.R_W) {     // Case (3):  send data to master
      I2C2TRN = bytes[rw];     // load slave's previously received data to send to master
      I2C2CONbits.SCLREL = 1;  // release the clock, allowing master to clock in data
    } else {                   // Case (4):  we have received data from the master
      bytes[rw] = I2C2RCV;     // store the received data byte
    }
    ++rw;                           
  } else {                     // the byte is an address byte, so Case (1) or (2)
    I2C5RCV;                   // read to clear I2C5RCV (we don't need our own address)
    if(I2C2STATbits.R_W) {     // Case (1):  send data to master
      I2C2TRN = bytes[rw];     // load slave's previously received data to send to master
      ++rw;
      I2C2CONbits.SCLREL = 1;  // release the clock, allowing master to clock in data
...
void i2c_slave_setup(unsigned char addr) { 
  I2C2ADD = addr;                   // the address of the slave
  IPC8bits.I2C2IP  = 1;             // slave has interrupt priority 1
  IEC1bits.I2C2SIE = 1;             // slave interrupt is enabled
  IFS1bits.I2C2SIF = 0;             // clear the interrupt flag
